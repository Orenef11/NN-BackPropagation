...
def training_neurons_network(network, training_samples, input_neurons, network_learning_rate,
                             neurons_activation_func_name, number_of_hidden_layer_neurons):
    epoch = 1
    stop_network_learning = False
    last_error_rate = 0
    while stop_network_learning is False:
        error_rate = 0
        for train_input in training_samples:
        ...
        with open(trained_network_weights_file_name, 'wb') as results_file:
            dump(network, results_file, protocol=HIGHEST_PROTOCOL)
